/* reader.cu - Read in data from matrix files
 *
...
/* ---------- Reading file data ---------- */
int read_input_file(const char *, Matrix *, Vector *);
...
int read_input_file(const char *input_file, Matrix *mat_A, Vector *vec_b)
{
  unsigned char ch;
  int curr_line = 0;
  ...
  mat_A->column_indices = col_indices;
  mat_A->row_pointers = row_ptr;
